...
class Solution {
    ...
    private int smoothPixel(int rowIndex, int columnIndex) {
        ...
        for (int modifiedRowIndex = Math.max(0, rowIndex - 1); modifiedRowIndex <= Math.min(rows - 1,
                rowIndex + 1); modifiedRowIndex++) {
            for (int modifiedColumnIndex = Math.max(0, columnIndex - 1); modifiedColumnIndex <= Math.min(columns - 1,
                    columnIndex + 1); modifiedColumnIndex++) {
