...
class Solution {
    public int[] relativeSortArray(int[] arr1, int[] arr2) {
        ...
        // now sort the leftovers - p points to the first element in series of those
        // from arr2 that are not in arr1
        Arrays.sort(res, p, res.length);//对数组部分排序下标从p到res.length-1
        return res;
    }
}
// @lc code=end
